...
const AddRecipeFormBase = () => {
  ...
  const handleRemoveItem = (index, object) => {
    setNewRecipe(prevState => {
      return {
        ...prevState,
        [object]: newRecipe[object].filter((item, secondIndex) => index !== secondIndex)
      };
    });
  };
  ...
  return (
    <FormContainer>
      <Form onSubmit={handleSubmit}>
        <label>
          Recipe title:
          <Input type="text" value={title} placeholder="title" id="title" onChange={handleChange} />
        </label>
        ...
        <ItemsList>
          ...
          <Button style={{ width: "30%" }} size="small" onClick={() => handleAddItem("ingredients")}>
            Add Ingredient
          </Button>
        </ItemsList>
        ...
        <ItemsList>
          ...
          <Button style={{ width: "30%" }} size="small" onClick={() => handleAddItem("cookingSteps")}>
            Add Step
          </Button>
        </ItemsList>
        ...
        <div style={{ marginBottom: "10px" }}>
          <label htmlFor={"nb of portions"}>Number of portions :</label>
          <Input
            type="number"
        ...
        <div style={{ display: "flex", justifyContent: "center" }}>
          <Checkbox onChange={handleChange} value={canBeFrozen}>
            can be frozen
          </Checkbox>
          <Checkbox onChange={event => handleChangeInNestedState(event, "categories", "vegan")} value={vegan}>
            vegan
          </Checkbox>
          <Checkbox onChange={event => handleChangeInNestedState(event, "categories", "healthy")} value={healthy}>
            healthy
          </Checkbox>
        </div>
        <ImageInput name="picture: " onChange={handleFile} required isLoading={isLoading} />
        <Button disabled={isLoading} type="submit" top="10px">
          SUBMIT RECIPE
        </Button>
      </Form>
    </FormContainer>
